...
- **Automatikmodus (🌡️)**: Neuer Punkt in der Modus-Auswahl — die Steuerung wählt anhand von Soll- und Raumtemperatur selbst Kühlen oder Heizen (Ziel darunter → Kühlen, darüber → Heizen; Wechsel im laufenden Betrieb erst ab 1 °C Differenz). Wird die Zieltemperatur bei ausgeschalteter Anlage verstellt, schaltet sie sich automatisch im Automatikmodus ein. Der Ad-hoc-Knopf benennt sich darin passend um (Ad-hoc Kühlen/Heizen). Der Betrieb-Schalter und die manuellen Modi bleiben unverändert; der Automatikmodus wird auf dem Gerät gespeichert, sodass alle Browser denselben Zustand zeigen
...
- **Wie eine App**: Web-App-Manifest + Icon — „Zum Startbildschirm hinzufügen" am Handy, und die Klimasteuerung startet mit eigenem Icon (auf iOS im Vollbild)
