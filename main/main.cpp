...
#include "htmls/static_gz.h"         // gzip blobs of css/js (generated by tools/gen_static_gz.py)
#include "htmls/html_common.h"       // common code HTML (like header, footer)
#include "htmls/icons.h"             // PWA app icons (generated by tools/gen_icons.py)
...
// filter runtime counter: ON-minutes since the last filter reset
uint32_t filter_minutes = 0;
uint16_t filter_threshold_hours = 100; // 0 = reminder disabled
uint32_t filter_minutes_persisted = 0;
...
float holiday_frost_temp = 16.0f; // celsius
bool frost_active = false;
unsigned long lastHolidayCheck = 0;
bool loadHoliday();
bool saveHoliday();
void checkHoliday();
void handleApiHoliday(AsyncWebServerRequest *request);
...
void checkNight()
{
  ...
  if (holiday_on)
    return; // holiday mode pauses the night automation (manual toggle still works)
  time_t now;
  time(&now);
...
bool loadFilter()
{
  if (!SPIFFS.exists(filter_conf))
  {
    return false;
  }
  File f = SPIFFS.open(filter_conf, "r");
  if (!f)
  {
    return false;
  }
  const size_t capacity = JSON_OBJECT_SIZE(2) + 64;
  DynamicJsonDocument doc(capacity);
  ...
  filter_minutes = doc["m"] | 0;
  filter_threshold_hours = doc["th"] | 100;
  filter_minutes_persisted = filter_minutes;
  return true;
}
...
bool saveFilter()
{
  const size_t capacity = JSON_OBJECT_SIZE(2) + 64;
  DynamicJsonDocument doc(capacity);
  doc["m"] = filter_minutes;
  doc["th"] = filter_threshold_hours;
  File f = SPIFFS.open(filter_conf, "w");
  if (!f)
  {
    return false;
  }
...
void checkFilter()
{
  if (lastFilterTick != 0 && millis() - lastFilterTick < 60000UL)
    return;
  ...
  {
    filter_minutes++;
    if (filter_minutes - filter_minutes_persisted >= 60)
    {
      saveFilter(); // persist once per operating hour to limit flash wear
    }
  }
}
...
void handleApiFilter(AsyncWebServerRequest *request)
{
  ...
  if (request->hasArg(F("th")))
  {
    int th = request->arg(F("th")).toInt();
    if (th < 0 || th > 1000)
    {
      request->send(400, "application/json", String(F("{\"err\":\"invalid\"}")));
      return;
    }
    filter_threshold_hours = (uint16_t)th;
    saveFilter();
    request->send(200, "application/json", String(F("{\"ok\":true}")));
    return;
  }
...
bool loadHoliday()
{
  if (!SPIFFS.exists(holiday_conf))
  {
    return false;
  }
  File f = SPIFFS.open(holiday_conf, "r");
  if (!f)
  {
    return false;
  }
  ...
  holiday_on = (doc["on"] | 0) != 0;
  holiday_frost = (doc["fp"] | 1) != 0;
  holiday_frost_temp = doc["ft"] | 16.0f;
  return true;
}
...
void checkHoliday()
{
  if (lastHolidayCheck != 0 && millis() - lastHolidayCheck < 60000UL)
    return;
  lastHolidayCheck = millis();
  if (!holiday_on)
  {
    frost_active = false;
    return;
  }
  if (!holiday_frost || !hp.isConnected())
    return;
  heatpumpSettings settings = hp.getSettings();
  float room = hp.getRoomTemperature();
  bool powerOn = settings.power != NULL && strcmp(settings.power, "ON") == 0;
  if (!frost_active && !powerOn && room > -40 && room < holiday_frost_temp)
  {
    settings.power = "ON";
    settings.mode = "HEAT";
    settings.temperature = holiday_frost_temp + 1.0f;
    nightSendSettings(settings);
    frost_active = true;
    ESP_LOGI(TAG, "Frost guard on (%.1fC)", room);
  }
  else if (frost_active && room > holiday_frost_temp + 1.5f)
  {
    settings.power = "OFF";
    nightSendSettings(settings);
    frost_active = false;
    ESP_LOGI(TAG, "Frost guard off (%.1fC)", room);
  }
}
...
void handleApiHoliday(AsyncWebServerRequest *request)
{
  ...
  {
    holiday_on = request->arg(F("on")) == "1";
    holiday_frost = request->arg(F("fp")) == "1";
    float ft = request->arg(F("ft")).toFloat();
    if (ft >= 5 && ft <= 25)
    {
      holiday_frost_temp = ft;
    }
    if (!holiday_on)
    {
      frost_active = false;
  ...
  json += holiday_on ? 1 : 0;
  json += F(",\"fp\":");
  json += holiday_frost ? 1 : 0;
  json += F(",\"ft\":");
  json += String(holiday_frost_temp, 1);
  json += F("}");
  request->send(200, "application/json", json);
}
...
void checkSchedules()
{
  ...
  if (holiday_on)
  {
    // holiday mode pauses all schedules; end windows that are still active
    for (uint8_t i = 0; i < schedules_count; i++)
    {
      if (schedules[i].active)
      {
        schedules[i].active = false;
        if (schedules[i].offAfter)
        {
          scheduleApply(schedules[i], false);
        }
      }
    }
    return;
...
void handleApiStatus(AsyncWebServerRequest *request)
{
  ...
  json += F(",\"hol\":");
  json += holiday_on ? F("true") : F("false");
  json += F(",\"fm\":");
  json += filter_minutes;
  json += F(",\"fth\":");
  json += filter_threshold_hours;
...
#ifdef KS_GZ_ASSETS
// css/js are compiled as gzip blobs (tools/gen_static_gz.py): about a quarter
// of the flash and transfer size, so connections close much sooner (ESP8266)
static void sendStaticGzP(AsyncWebServerRequest *request, const String& contentType, const uint8_t *data, size_t len)
{
...
{
#ifdef KS_GZ_ASSETS
  sendStaticGzP(request, F("text/css"), css_gz, css_gz_len);
#else
...
void handleManifest(AsyncWebServerRequest *request)
{
  sendStaticP(request, F("application/manifest+json"), html_manifest);
}

static void sendIcon(AsyncWebServerRequest *request, const uint8_t *data, size_t len)
{
  AsyncWebServerResponse *response = request->beginResponse_P(200, F("image/png"), data, len);
...
void handleIcon192(AsyncWebServerRequest *request)
{
  sendIcon(request, icon_192_png, icon_192_png_len);
}

void handleIcon512(AsyncWebServerRequest *request)
{
  sendIcon(request, icon_512_png, icon_512_png_len);
}

void handleControlJs(AsyncWebServerRequest *request)
{
#ifdef KS_GZ_ASSETS
  sendStaticGzP(request, F("application/javascript"), control_js_gz, control_js_gz_len);
#else
...
{
#ifdef KS_GZ_ASSETS
  sendStaticGzP(request, F("application/javascript"), timers_js_gz, timers_js_gz_len);
#else
...
{
#ifdef KS_GZ_ASSETS
  sendStaticGzP(request, F("application/javascript"), devices_js_gz, devices_js_gz_len);
#else
